...
export class ShoppingCartComponent {
  ...
  ngOnInit(): void {
    ...
    this.adminService.getAccounts().subscribe({
      next: (accounts: Account[]) => {
        this.account = accounts.filter((account) => account.id === userId);
      },
      error: (error) => {
        // Handle errors here
  ...
  loadShoppingCart(userId: number): void {
    this.marketplaceService.getShoppingCart(userId).subscribe((cart) => {
  ...
  removeOrderItem(
    ...
    this.marketplaceService.removeOrderItem(cartId, tourId).subscribe(
      (updatedShoppingCart) => {
  ...
  purchase(cartId: number): void {
    ...
    this.marketplaceService.updateShoppingCart(this.shoppingCart).subscribe(
      (updatedShoppingCart) => {
        this.profile.balance -= this.shoppingCart.total;
        this.shoppingCart = updatedShoppingCart;
        this.shoppingCart.orderItems.length = 0;
        this.adminService
          .updateProfile(this.profile, this.profile.userId)
          .subscribe({
            next: (profile: Profile) => {
              this.profile = profile;
            },
    ...
    this.marketplaceService.purchase(cartId).subscribe(
      (updatedShoppingCart) => {
        this.shoppingCart = updatedShoppingCart;
          ...
          this.tourService.getTourByGuide(coupon.authorId, 1, 100).subscribe({
            next: (result: PagedResults<Tour>) => {
              if (!result || result.totalCount === 0) {
                alert(
                  'The coupon you entered does not belong to the author of this tour!'
                );
                return;
              }
              ...
              if (this.isValidAuthor) {
                ...
                for (const orderItem of this.shoppingCart.orderItems) {
                  if (orderItem.idTour === tourId) {
                    orderItem.price -=
                      (coupon.discount / 100) * orderItem.price;
                  }
                }
